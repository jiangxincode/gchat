                                        ...
                                        gpointer         user_data)
{
	...
	gifdataS = (char *) malloc(strlen(gtk_entry_get_text(GTK_ENTRY(loginid))) + strlen(gtk_entry_get_text(GTK_ENTRY(password))) + 5);
	...
	if(pthread_create( &pthd, NULL, (void *)gif_receive_messages, (int *)sockfd) != 0)
	{
		errormsg = strerror(errno);
		gtk_widget_show(create_msgbox("error", errormsg));
	}
		...
		switch(gifheader->type)
		{
			case 3: // GIF_ADDRLIST_MSG
			{
				...
				while(counter < (gifheader->length))
				{
					...
					if(usrs->status == 1)
						...
						img = gdk_pixbuf_new_from_file ("../pixmaps/kill.png", NULL);
					gdk_threads_leave();
			...
			case 6: // GIF_CHAT_MSG
			{
				...
				if(ptr->window_open == 0)
				{
					...
					widget = lookup_widget(chat_window[ptr->window_id], "entInput");
					gtk_window_set_focus(GTK_WINDOW(chat_window[ptr->window_id]), widget);
					gtk_window_set_title(GTK_WINDOW(chat_window[ptr->window_id]), gifheader->sender);
					gtk_widget_show(chat_window[ptr->window_id]);	
					gdk_threads_leave();

					// creating tags(foreground color) for the buffer
					gdk_threads_enter();
					widget = lookup_widget(chat_window[ptr->window_id], "txtDisplay");
					buff = gtk_text_view_get_buffer(GTK_TEXT_VIEW(widget));
					gtk_text_buffer_create_tag (buff, "red_foreground", "foreground", "red", NULL);  
					gtk_text_buffer_create_tag (buff, "blue_foreground", "foreground", "blue", NULL);  
					gdk_threads_leave();
				}
			...
			case 8: // GIF_SUCCESS_N_ERROR_MSG
			{
				switch(gifheader->reserved)
				{
					...
					{
						gdk_threads_enter();
						gtk_widget_show(create_msgbox("error", "Login Incorrect"));
						gdk_threads_leave();
					...
					case 102: // GIF_SUCCESS_ADD_CONTACTS
					{
						...
						gdk_threads_enter();
						gtk_widget_show(create_msgbox("info", "Your new contact has been successfully added"));	
						gdk_threads_leave();
					...
					{
						gdk_threads_enter();
						gtk_widget_show(create_msgbox("error", "The contact id you entered does not belong to a gchat user"));		
						gdk_threads_leave();
					...
					{
						gdk_threads_enter();
						gtk_widget_show(create_msgbox("info", "Deleted. See the new list after you re-login"));		
						gdk_threads_leave();
					...
					{
						gdk_threads_enter();
						gtk_widget_show(create_msgbox("error", "1. The id you entered is not in your contact list"));		
						gdk_threads_leave();
					...
					{
						gdk_threads_enter();
						gtk_widget_show(create_msgbox("error", "2. The id you entered is not in your contact list"));		
						gdk_threads_leave();
			...
			case 10: // GIF_OFFLINE_MESSAGE
			{
				...
				while(counter < (gifheader->length))
				{
					...
					memcpy(omsgs_se, (gifdata + counter), sizeof(offline_msgs_send_t));
					...
					gtk_list_store_set (store, &iter, OFFLINE_NEW_PIXMAP, img, OFFLINE_SENDER_TEXT, omsgs_se->sender, OFFLINE_DATESERIAL_TEXT, omsgs_se->dateserial, OFFLINE_MSG_TEXT, message, -1);
					gdk_threads_leave(); 
				...
				if(gifheader->reserved == 1)
				{
					...
					// signal handling for "changed" event
					gdk_threads_enter();
					select = gtk_tree_view_get_selection (GTK_TREE_VIEW (offline_tree));
					gtk_tree_selection_set_mode (select, GTK_SELECTION_SINGLE);
					g_signal_connect (G_OBJECT (select), "changed", G_CALLBACK (gif_select_offline_messages), NULL);
					gdk_threads_leave();
				}
...
{
        if (gtk_tree_selection_get_selected (selection, &offline_selected_model, &offline_selected_iter))
        {
...
gif_call_client_for_chat(GtkTreeSelection *selection, gpointer data)
{
	contacts_chat_window_id_t *ptr;
        ...
        {
                gtk_tree_model_get (model, &iter, CONTACTS_COLUMN_TEXT, &selected_name, -1);
  
		if((strcmp(selected_name, "Available")) != 0 && (strcmp(selected_name, "Not Available")) != 0)
		{
			for(ptr = head; ptr != NULL; ptr = ptr->next)
			{
			...
			if(ptr->window_open == 0)
			{
				...
				widget = lookup_widget(chat_window[ptr->window_id], "entInput");
				gtk_window_set_focus(GTK_WINDOW(chat_window[ptr->window_id]), widget);
				gtk_window_set_title(GTK_WINDOW(chat_window[ptr->window_id]), selected_name);
				gtk_widget_show(chat_window[ptr->window_id]);	
                                        ...
                                        gpointer         user_data)
{
	...
	gifdataS = (char *) malloc(strlen(gtk_entry_get_text(GTK_ENTRY(input_text))));
                                        ...
                                        gpointer         user_data)
{
	...
	if(strcmp(gtk_window_get_title(GTK_WINDOW(widget)), "gchat - Add contacts") == 0)
	...
	gifdataS = (char *) malloc(strlen(gtk_entry_get_text(GTK_ENTRY(contactsid))));
                                        ...
                                        gpointer         user_data)
{
	...
	gtk_tree_model_get (offline_selected_model, &offline_selected_iter, OFFLINE_MSG_TEXT, &offline_selected_msg, -1);
